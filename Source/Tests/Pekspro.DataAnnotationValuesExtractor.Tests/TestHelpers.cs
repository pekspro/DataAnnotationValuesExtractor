...
internal static class TestHelpers
{
    ...
    public static (ImmutableArray<Diagnostic> Diagnostics, string[] Output) GetGeneratedTrees<TExtractor, TTrackingNames>(params string[] sources)
    where TExtractor : IIncrementalGenerator, new()
    {
        ...
        return GetGeneratedTrees<TExtractor>(sources, trackingNames);
    }

    public static (ImmutableArray<Diagnostic> Diagnostics, string[] Output) GetGeneratedTrees<T>(string[] source, params string[] stages)
        where T : IIncrementalGenerator, new()
    {
        return GetGeneratedTrees<T>(source, true, stages);
    }

    public static (ImmutableArray<Diagnostic> Diagnostics, string[] Output) GetGeneratedTrees<T>(string[] source, bool assertOutput, params string[] stages)
        where T : IIncrementalGenerator, new()
    {
        var preprocessorSymbols = new List<string> { "DEBUG" };
        ...
        var additionalReferences = new List<MetadataReference>
        {
            ...
            MetadataReference.CreateFromFile(typeof(DataAnnotationValuesOptionsAttribute).Assembly.Location),
            MetadataReference.CreateFromFile(typeof(StringLengthAttribute).Assembly.Location)
        };
        
        var references = baseReferences.Concat(additionalReferences);
        ...
        GeneratorDriverRunResult runResult = RunExtractorAndAssertOutput<T>(compilation, stages, assertOutput);

        return (runResult.Diagnostics, runResult.GeneratedTrees.Select(x => x.ToString()).ToArray());
    }

    private static GeneratorDriverRunResult RunExtractorAndAssertOutput<T>(CSharpCompilation compilation, string[] trackingNames, bool assertOutput = true)
    where T : IIncrementalGenerator, new()
    {
